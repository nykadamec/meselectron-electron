  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-surface-base border border-border-base rounded-xl shadow-elevated animate-slide-in z-50 focus:outline-none">
          <div className="p-6">
            {/* Header */}
            ...
            {/* Video Preview */}
            <div className="flex items-center gap-3 p-3 bg-bg-hover rounded-lg mb-6">
              <div className="w-20 h-12 bg-surface-elevated rounded overflow-hidden flex-shrink-0">
                {video.thumbnail ? (
                  <img
                    src={video.thumbnail}
                    alt={video.title}
              ...
              <div className="flex-1 min-w-0">
                ...
                {video.size && (
                  <p className="text-xs text-text-muted">{video.size}</p>
                )}
              </div>
            </div>
